...
def scaled_dot_product_attention(keys: tf.Tensor, queries: tf.Tensor, values: tf.Tensor):
    """
    ...
    attention = tf.matmul(queries, keys)  # (batch size, sequence length q, sequence length k)
    attention = attention / tf.sqrt(tf.cast(tf.shape(keys)[2], dtype=tf.float32))  # scaling
...
class ChessModel(tf.keras.Model):
    def __init__(self, num_layers, depth, d_fnn):
        ...
        self.encoding = tf.keras.layers.Dense(depth, use_bias=False)
        self.chess_layers = [ChessModelLayer(depth, d_fnn) for _ in range(num_layers)]

    def call(self, board, metadata, legal_moves):
        """
        calls the evaluator
        ...
        metadata = tf.reshape(metadata, [-1, 1, 1, METADATA_LENGTH])  # (batch size, 1, 1, METADATA)
        metadata += tf.zeros([1, ROWS, COLS, 1], dtype=tf.int32)  # broadcast to (batch size, ROWS, COLS, METADATA)
        board = tf.concat([board, metadata], axis=3)  # (batch size, ROWS, COLS, depth_0), type = int
