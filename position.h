...
Move* Position::generate_legals(Move* list) {
	if (num_ply_no_capture_or_pawn_move() >= 2 * max_moves_without_pawn_move_or_captures
		|| (position_history.count(get_hash()) == 1 && position_history[get_hash()] >= 3)) // the game is a draw.
		return list;
